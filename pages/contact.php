...
function showContactForm($data)
{
    ...
    <div>
        ...
        <select id="salutation" name="salutation" required>
            ...
            <option value="mrs" ';
    if (isset($data['salutation']) && $data['salutation'] == "mrs") echo 'selected';
    echo '>Mevr.</option>
            <option value="mr" ';
    if (isset($data['salutation']) && $data['salutation'] == "mr") echo 'selected';
    echo '>Dhr.</option>
            <option value="mx" ';
    if (isset($data['salutation']) && $data['salutation'] == "mx") echo 'selected';
    echo '>Mx.</option>
            <option value="undisclosed" ';
    if (isset($data['salutation']) && $data['salutation'] == "undisclosed") echo 'selected';
    echo '>Zeg ik liever niet</option>
        </select>';
    echo '<span class="error">';
    echo $data['salutationErr'] ?? '';
    echo '</span>
                </div> 
                <div>     
    ...
    echo ' ">
                        <span class="error"> ';
    echo $data['housenumberErr'] ?? '';
    echo ' </span>
                    </div>
                    <div>
                        <label for="housenumberAddition">Toevoegingen:</label> 
                        <input type="text" id="housenumberAddition" name="housenumberAddition" value=" ';
    echo $data['housenumberAddition'] ?? '';
    echo ' ">
                        <span class="error"> ';
    echo $data['housenumberAdditionErr'] ?? '';
    echo ' </span>
                    </div>
                    <div>
                        <label for="postalcode">Postcode:</label>
                        <input type="text" id="postalcode" name="postalcode" value=" ';
    echo $data['podstalcode'] ?? '';
    echo ' ">
                        <span class="error"> ';
    echo $data['postalcodeErr'] ?? '';
    echo ' </span>
                    </div>
                    <div>
    ...
    if (!empty($data['commPreference']) && ($data['commPreference'] == "phone")) echo 'checked';
    echo '>
                            <label for="commPreferencePhone">Telefoon</label>
                            <span class="error">';
    echo $data['commPreferenceErr'] ?? '';
    echo '</span>
                        </li>
                        <li>
                            <input type="radio" id="commPreferencePost" name="commPreference" value="post" ';
    if (!empty($data['commPreference']) && ($data['commPreference']) == "post") echo 'checked';
    echo ' >
    ...
    echo '<div>
                ...
                <div>
                    <textarea name="message" rows="10" cols="30" placeholder="Type hier je bericht..."> ';
    echo $data['message'] ?? '';
    echo '</textarea>
                    <span class="error"> ';
    echo $data['messageErr'] ?? '';
    echo ' </span>
                </div> ';
    //<!--Submit button-->
